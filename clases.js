 ...
 * @param {string} classId - Unique class ID
 * @param {string} creatorId - Creator's ID
 * @param {string} creationDate - Creation date as string
 * @returns {Promise<void>}
 */
export async function createClass(driver, classId, creatorId, creationDate) {
  const session = driver.session();
  try {
    await session.run(
      'CREATE (c:Class {classId: $classId, creatorId: $creatorId, creationDate: $creationDate})',
      { classId, creatorId, creationDate }
    );
  } finally {
    await session.close();
  }
 ...
 * @returns {Promise<void>}
 */
export async function addEvaluation(driver, classId, evalId, name, type, content) {
  const session = driver.session();
  try {
    await session.run(
      `MATCH (c:Class {classId: $classId})
       CREATE (e:Evaluation {evalId: $evalId, name: $name, type: $type, content: $content}),
              (c)-[:HAS_EVALUATION]->(e)`,
      { classId, evalId, name, type, content }
    );
  } finally {
    await session.close();
  }
 ...
 * @returns {Promise<void>}
 */
export async function addStudent(driver, classId, studentId) {
  const session = driver.session();
  try {
    await session.run(
 ...
 * @param {neo4j.Driver} driver - The Neo4j driver
 * @param {string} parentId - Parent class ID or section ID
 ...
 * @returns {Promise<void>}
 */
export async function addSection(driver, parentId, sectionId, description, isClassParent = true) {
  const session = driver.session();
  try {
    if (isClassParent) {
      await session.run(
        `MATCH (c:Class {classId: $parentId})
 ...
 */
export async function getClassDetails(driver, classId) {
  const session = driver.session();
  try {
 ...
 */
export async function getEvaluations(driver, classId) {
  const session = driver.session();
  try {
    const result = await session.run(
      `MATCH (c:Class {classId: $classId})-[:HAS_EVALUATION]->(e:Evaluation)
       RETURN e`,
      { classId }
    );
 ...
 */
export async function getStudents(driver, classId) {
  const session = driver.session();
  try {
    const result = await session.run(
      `MATCH (c:Class {classId: $classId})-[:HAS_STUDENT]->(s:Student)
       RETURN s`,
      { classId }
    );
 ...
 * @param {neo4j.Driver} driver - The Neo4j driver
 * @param {string} parentId - Class ID or Section ID
...
export async function getSections(driver, parentId, isClass = true) {
  ...
  try {
    let query;
    if (isClass) {
      query = `MATCH (c:Class {classId: $parentId})-[:HAS_SECTION]->(s:Section)
               RETURN s`;
    } else {
...
export async function sendSampleData(driver) {
  try {
    ...
    await createClass(driver, sampleData.class.classId, sampleData.class.creatorId, sampleData.class.creationDate);

    // Add evaluations
    console.log('Adding evaluations...');
    for (const evaluation of sampleData.evaluations) {
      await addEvaluation(driver, sampleData.class.classId, evaluation.evalId, evaluation.name, evaluation.type, evaluation.content);
    }

    // Add students
    console.log('Adding students...');
    for (const student of sampleData.students) {
      await addStudent(driver, sampleData.class.classId, student.studentId);
    }

    // Add sections with subsections
    console.log('Adding sections...');
    for (const section of sampleData.sections) {
      await addSection(driver, sampleData.class.classId, section.sectionId, section.description, true);

      // Add subsections
      if (section.subsections) {
        for (const subsection of section.subsections) {
          await addSection(driver, section.sectionId, subsection.sectionId, subsection.description, false);
        }
      }
    }
